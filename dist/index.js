  ...
  const LauncherInstallModal = ({ closeModal, launcherOptions, serverAPI }) => {
      ...
      return ((progress.status != '' && progress.percent < 100) ?
          ...
          window.SP_REACT.createElement(deckyFrontendLib.ModalRoot, { onCancel: closeModal },
              ...
              window.SP_REACT.createElement("p", { style: { fontSize: 'small', marginTop: '18px' } }, "Note: When installing a launcher, the latest Proton-GE will attempt to be installed. If your launchers don't start, make sure force compatibility is checked, shortcut properties are right, and your steam files are updated. Remember to also edit your controller layout configurations if necessary! If all else fails, restart your steam deck manually."),
              window.SP_REACT.createElement("p", { style: { fontSize: 'small', marginTop: '18px' } }, "Note\u00B2: Some games won't run right away using NSL. Due to easy anti-cheat or quirks, you may simply need to manually tinker to get some games working. NSL is simply another way to play! Happy Gaming!"),
  ...
  const StreamingInstallModal = ({ closeModal, streamingOptions, serverAPI }) => {
      ...
      return ((progress.status != '' && progress.percent < 100) ?
          ...
          window.SP_REACT.createElement(deckyFrontendLib.ModalRoot, { onCancel: closeModal },
              ...
              window.SP_REACT.createElement(deckyFrontendLib.DialogBodyText, null, "NSL will install and use Chrome to launch these sites. Non-Steam shortcuts will be created for each selection. Toggle Auto Scan \"on\" for these."),
