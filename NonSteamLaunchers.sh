...
if [ ${#args[@]} -gt 0 ]; then
    # If there are command line arguments, kill the specific instance of Steam
    pkill -f '/usr/bin/steam'
    
    # Wait for the specific instance of Steam to be closed
    while pgrep -f '/usr/bin/steam' > /dev/null; do sleep 1; done
else
