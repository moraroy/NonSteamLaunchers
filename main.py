        ...
        for option, is_selected in selected_options.items():
            if is_selected:
                if option in ['xboxGamePass', 'geforceNow', 'amazonLuna', 'netflix', 'hulu', 'disneyPlus', 'amazonPrimeVideo', 'youtube']:
                    ...
                    # Launcher option
                    selected_option = camel_to_title(option).replace('Ea App', 'EA App').replace('Gog Galaxy', 'GOG Galaxy').replace('Battle Net', 'Battle.net').replace('Itch Io', 'itch.io').replace('Humble Games', 'Humble Games Collection').replace('Indie Gala', 'IndieGala').replace('Rockstar', 'Rockstar Games Launcher').replace('Glyph', 'Glyph Launcher').replace('Ps Plus', 'Playstation Plus').replace('DMM', 'DMM Games').replace('Separate App IDs', 'SEPARATE APP IDS - CHECK THIS TO SEPARATE YOUR PREFIX'S')
                    if ' ' in selected_option:
                        selected_option = f'"{selected_option}"'
                    selected_options_list.append(selected_option)
